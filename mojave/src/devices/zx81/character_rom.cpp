...
uint8_t Zx81CharacterRom::glyphIndex(uint8_t code) {
    return static_cast<uint8_t>(code & kGlyphIndexMask);
}

bool Zx81CharacterRom::isInverted(uint8_t code) {
    return (code & kInvertBit) != 0;
}

uint16_t Zx81CharacterRom::glyphRowAddress(uint8_t code, uint8_t row) {
    return static_cast<uint16_t>(kCharsetBase + static_cast<uint16_t>(glyphIndex(code)) * kGlyphRows + row);
}

uint8_t Zx81CharacterRom::readRow(ReadFn read, uint8_t code, uint8_t row) {
...
std::array<uint8_t, Zx81CharacterRom::kGlyphRows> Zx81CharacterRom::readGlyph(ReadFn read, uint8_t code) {

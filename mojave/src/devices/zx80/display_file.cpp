...
std::optional<Zx80DisplayFileGrid> Zx80DisplayFile::parse(ReadFn read, uint16_t dfile_start) {
    if (!read)
        return std::nullopt;

    uint16_t pc = dfile_start;
    if (read(pc) != kHaltOpcode)
        return std::nullopt;
    pc++;

    Zx80DisplayFileGrid grid{};
    for (unsigned row = 0; row < kRows; ++row) {
        Zx80DisplayFileLine& line = grid.rows[row];
        line.length = 0;

        while (line.length < kColumns) {
            const uint8_t value = read(pc);
            if (value == kHaltOpcode)
                break;
...
uint16_t Zx80DisplayFile::readPointer(ReadFn read, uint16_t pointer_address) {
    if (!read)
        return 0;
...
void Zx80DisplayFile::writePointer(WriteFn write, uint16_t pointer, uint16_t pointer_address) {
    if (!write)
        return;
...
uint16_t Zx80DisplayFile::writeCollapsed(WriteFn write, uint16_t dfile_start) {
    for (unsigned i = 0; i < kMinCollapsedBytes; ++i)
        write(static_cast<uint16_t>(dfile_start + i), kHaltOpcode);

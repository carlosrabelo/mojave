...
uint8_t Trs80M3Keyboard::read(uint16_t address) {
...
void Trs80M3Keyboard::write(uint16_t /*address*/, uint8_t /*value*/) {}

void Trs80M3Keyboard::reset() {
    rows_.fill(0);
}

void Trs80M3Keyboard::setKeyDown(uint8_t row, uint8_t bit, bool down) {
...
bool Trs80M3Keyboard::keyDown(uint8_t row, uint8_t bit) const {
    if (row >= kRowCount || bit >= kBitsPerRow)
        return false;
    return (rows_[row] & (1u << bit)) != 0;
}

void Trs80M3Keyboard::releaseAll() {
    rows_.fill(0);
}

bool Trs80M3Keyboard::lookupNamedKey(char name, uint8_t& row, uint8_t& bit) {
    if (name >= 'a' && name <= 'z')
...
bool Trs80M3Keyboard::namedKeyBit(char name, uint8_t& row, uint8_t& bit) {
    return lookupNamedKey(name, row, bit);
}

bool Trs80M3Keyboard::specialKeyBit(SpecialKey key, uint8_t& row, uint8_t& bit) {
    switch (key) {
...
bool Trs80M3Keyboard::pressNamedKey(char name) {
    uint8_t row = 0;
    uint8_t bit = 0;
...
void Trs80M3Keyboard::releaseNamedKey(char name) {
    uint8_t row = 0;
    uint8_t bit = 0;
    if (!lookupNamedKey(name, row, bit))
        return;
...
void Trs80M3Keyboard::pressSpecialKey(SpecialKey key) {
    uint8_t row = 0;
    uint8_t bit = 0;
...
void Trs80M3Keyboard::releaseSpecialKey(SpecialKey key) {
    uint8_t row = 0;
    uint8_t bit = 0;
    if (!specialKeyBit(key, row, bit))
        return;

...
bool unshiftedNamed(char name, Trs80M1L1TypingChord& out) {
    if (!Trs80M1Keyboard::namedKeyBit(name, out.row, out.bit))
        return false;
    out.shift = false;
    return true;
}

bool shiftedKey(uint8_t row, uint8_t bit, Trs80M1L1TypingChord& out) {
    out.row = row;
    out.bit = bit;
    out.shift = true;
    return true;

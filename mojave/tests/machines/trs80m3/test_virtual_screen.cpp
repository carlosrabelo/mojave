...
TEST_CASE("TRS-80 Model III preset is eligible for SDL and Qt6 frontends",
          "[machine][trs80m3][integration]") {
    const BuiltinPresetDescriptor* preset = findBuiltinPreset("trs80m3");
    REQUIRE(preset != nullptr);
    REQUIRE(preset->needs_virtual_screen);

    auto machine = createBuiltinMachine("trs80m3");
...
TEST_CASE("TRS-80 Model III virtual screen shows VRAM through the screen registry",
          "[machine][trs80m3][integration]") {
    ...
    machine->bus().write(Trs80M3PresetContract::vram_start + 1, 'a');

    Framebuffer* screen = registry.screen("main");
    REQUIRE(screen == fb);
    REQUIRE(screen->getPixel(3, 2) == 0xFFFFFFFFu);
    REQUIRE(screen->getPixel(11, 4) == 0xFFFFFFFFu);
}

TEST_CASE("TRS-80 Model III frontend frame loop uses guest clock pacing",
          "[machine][trs80m3][integration]") {
    auto machine = createTrs80M3Machine();
    const BuiltinPresetDescriptor* preset = findBuiltinPreset("trs80m3");
    REQUIRE(preset != nullptr);
    ...
    machine->bus().write(Trs80M3PresetContract::ram_start, 0x18);
    machine->bus().write(Trs80M3PresetContract::ram_start + 1, 0xFE);
    dynamic_cast<Z80&>(machine->cpu()).regs().pc = Trs80M3PresetContract::ram_start;

    const unsigned cycles = runner::stepForFrame(*machine, clock);
    REQUIRE(cycles == clock.cyclesPerFrame());
    REQUIRE(cycles == 33792u);
}

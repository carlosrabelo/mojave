...
TEST_CASE("TRS-80 Model I Level II preset is eligible for SDL and Qt6 frontends",
          "[machine][trs80m1l2][integration]") {
    const BuiltinPresetDescriptor* preset = findBuiltinPreset("trs80m1l2");
    REQUIRE(preset != nullptr);
    REQUIRE(preset->needs_virtual_screen);

    auto machine = createBuiltinMachine("trs80m1l2");
...
TEST_CASE("TRS-80 Model I Level II virtual screen shows VRAM through the screen registry",
          "[machine][trs80m1l2][integration]") {
    ...
    machine->bus().write(Trs80M1L2PresetContract::vram_start + 1, 'B');
...
TEST_CASE("TRS-80 Model I Level II frontend frame loop uses guest clock pacing",
          "[machine][trs80m1l2][integration]") {
    auto machine = createTrs80M1L2Machine();
    const BuiltinPresetDescriptor* preset = findBuiltinPreset("trs80m1l2");
    REQUIRE(preset != nullptr);
    ...
    machine->bus().write(Trs80M1L2PresetContract::ram_start, 0x18);
    machine->bus().write(Trs80M1L2PresetContract::ram_start + 1, 0xFE);
    dynamic_cast<Z80&>(machine->cpu()).regs().pc = Trs80M1L2PresetContract::ram_start;
